...
Write-Host ""
Write-Host "=== NIFDU APP WRITER SIMPLE ===" -ForegroundColor Yellow
...
try {
    ...
    Write-Host "[ERROR] HTTP error calling /api/codegen: $($_.Exception.Message)" -ForegroundColor Red
    exit 1
}
...
if (-not $files -or $files.Count -eq 0) {
    Write-Host "[FILES] No files[] returned from /api/codegen, nothing to write." -ForegroundColor DarkYellow
} else {
    Write-Host ""
    Write-Host "[FILES] Writing $($files.Count) file(s)..." -ForegroundColor Cyan
    ...
    foreach ($f in $files) {
        ...
        if (-not $path) {
            Write-Host "  [SKIP] File with no path." -ForegroundColor DarkYellow
            continue
        }
...
# --------------------------------------------------------
Write-Host ""
Write-Host "[STEP 2] Building in $BuildDir" -ForegroundColor Yellow
